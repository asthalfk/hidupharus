# generate token using invalid credential both username and password value
...
def generate_token_failed():
    try :
        response = requests.post(url=object_data.link+"/api/auth/token", auth=(object_data.invalid_username, object_data.invalid_password))
